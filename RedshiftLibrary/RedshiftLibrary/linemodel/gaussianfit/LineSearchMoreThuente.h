...
#ifndef LBFGSPP_LINE_SEARCH_MORE_THUENTE_FORK_H
#define LBFGSPP_LINE_SEARCH_MORE_THUENTE_FORK_H
...
///
/// The line search algorithm by Moré and Thuente (1994), currently used for the
/// L-BFGS-B algorithm.
///
/// The target of this line search algorithm is to find a step size \f$\alpha\f$
/// that satisfies the strong Wolfe condition \f$f(x+\alpha d) \le f(x) +
/// \alpha\mu g(x)^T d\f$ and \f$|g(x+\alpha d)^T d| \le \eta|g(x)^T d|\f$. Our
/// implementation is a simplified version of the algorithm in [1]. We assume
/// that \f$0<\mu<\eta<1\f$, while in [1] they do not assume \f$\eta>\mu\f$. As
/// a result, the algorithm in [1] has two stages, but in our implementation we
...
///
template <typename Scalar> class LineSearchMoreThuenteFork {
private:
  ...
  static Scalar quadratic_minimizer(const Scalar &a, const Scalar &b,
                                    const Scalar &fa, const Scalar &ga,
                                    const Scalar &fb) {
    const Scalar ba = b - a;
  ...
  // Local minimizer of a cubic function q(x) = c0 + c1 * x + c2 * x^2 + c3 *
  // x^3 that interpolates fa, ga, fb and gb, assuming a != b Also sets a flag
  // indicating whether the minimizer exists
  static Scalar cubic_minimizer(const Scalar &a, const Scalar &b,
    ...
    // The minimizer is a solution to the equation c1 + 2*c2 * x + 3*c3 * x^2 =
    // 0 roots = -(z2/z3) / 3 (+-) sqrt((z2/z3)^2 - 3 * (z1/z3)) / 3
    //
  ...
  static Scalar step_selection(const Scalar &al, const Scalar &au,
                               const Scalar &at, const Scalar &fl,
                               const Scalar &fu, const Scalar &ft,
                               const Scalar &gl, const Scalar &gu,
                               const Scalar &gt) {
    using std::abs;
    ...
    bool ac_exists;
    // std::cout << "al = " << al << ", at = " << at << ", fl = " << fl << ", ft
    // = " << ft << ", gl = " << gl << ", gt = " << gt << std::endl;
    ...
    if (abs(gt) < abs(gl)) {
      ...
      const Scalar res = (ac_exists && (ac - at) * (at - al) > Scalar(0) &&
                          abs(ac - at) < abs(as - at))
                             ? ac
                             : as;
      // Postprocessing the chosen step
      return (at > al) ? std::min(at + deltau * (au - at), res)
    ...
    // The following is not used in the paper, but it seems to be a reasonable
    // safeguard
    return (at > al) ? std::min(at + deltau * (au - at), ae)
...
public:
  ...
  static void LineSearch(Foo &f, const SolverParam &param, const Vector &xp,
                         const Vector &drt, const Scalar &step_max,
                         Scalar &step, Scalar &fx, Vector &grad, Scalar &dg,
                         Vector &x) {
    using std::abs;
    ...
    const auto grad_init = grad;

    // std::cout << "fx_init = " << fx_init << ", dg_init = " << dg_init <<
    // std::endl << std::endl;

    // Make sure d points to a descent direction
    if (dg_init >= Scalar(0))
      throw std::logic_error("the moving direction does not decrease the "
    ...
    // std::cout << "max_step = " << step_max << ", step = " << step << ", fx =
    // " << fx << ", dg = " << dg << std::endl;
    ...
    for (iter = 0; iter < param.max_linesearch; iter++) {
      ...
      if (ft > fI_lo) {
        // Case 1: ft > fl
        new_step = step_selection(I_lo, I_hi, step, fI_lo, fI_hi, ft, gI_lo,
                                  gI_hi, gt);
        ...
        // First let step_selection() decide a value, and then project to the
        // range above
        //
        // new_step = step_selection(I_lo, I_hi, step, fI_lo, fI_hi, ft, gI_lo,
        // gI_hi, gt); const Scalar delta2 = Scalar(4) const Scalar t1 = step +
        // delta * (step - I_lo); const Scalar t2 = step + delta2 * (step -
        // I_lo); const Scalar tl = std::min(t1, t2), tu = std::max(t1, t2);
      ...
      // In case step, new_step, and step_max are equal, directly return the
      // computed x and fx
      ...
      // std::cout << "step = " << step << ", fx = " << fx << ", dg = " << dg <<
      // std::endl;
      ...
      // the best one so far. If not, go to the next iteration and find a better
      // one
      if (step >= step_max) {
        const Scalar ft = fx - fx_init - step * test_decr;
        if (ft <= fI_lo) {
    ...
    if (iter >= param.max_linesearch) {
      // original code
      // throw std::runtime_error("the line search routine reached the maximum
...
#endif // LBFGSPP_LINE_SEARCH_MORE_THUENTE_FORK_H
