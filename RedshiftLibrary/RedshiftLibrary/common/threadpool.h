...
public:
    ...
    Bool IsRunning()
    {
        for(std::list<boost::thread*>::iterator it=m_Threads.begin(),end=m_Threads.end(); it!=end; ++it)
        {
            if ((*it)->joinable())
            {
                if( (*it)->timed_join( boost::posix_time::time_duration(0, 0, 0) ) == false )
                {
                    return true;
                }
            }
