...
void CLineModelElementList::LoadFitContinuumOneTemplate(const TFloat64Range& lambdaRange, const CTemplate& tpl){
  ...
  Log.LogInfo("LMfit : Continuum %s amplitude is set", fitContinuumAmplitudeNegative ? "negative" : "positive");
...
Int32 CLineModelElementList::LoadFitContaminantTemplate(const TFloat64Range& lambdaRange, const CTemplate& tpl){
    ...
    if(chisquareResult->Redshifts.size()>extremumCount)
    {
        ...
        Log.LogDebug("  EltList: EXTREMA found n=%d", extremumList.size());
    }else
    {
        // store extrema results
        ...
        Log.LogDebug("  EltList: EXTREMA forced n=%d", chisquareResult->Extrema.size());
    }    
