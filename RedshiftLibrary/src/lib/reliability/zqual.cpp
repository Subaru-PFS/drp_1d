...
std::shared_ptr<const CQualzResult> CQualz::Compute( CDataStore& resultStore, CClassifierStore& classifierStore,
...
Bool CQualz::ExtractFeaturesPDF( CDataStore& resultStore, const TFloat64Range& redshiftRange, Float64& redshiftStep )
{
    ...
    if(!logzpdf1d)
    {
        Log.LogError( "ExtractFeaturesPDF: no results retrieved from scope: %s", scope_res.c_str());
        return false;
    }
	...
	if ( nanVector ) {
		...
		extremum.Find( logzpdf1d->Redshifts, logzpdf1d->valProbaLog, extremumList ); // already sorted list in logPZ in output
		//extremum.Find( logzpdf1d->Redshifts, zpdf1d, extremumList ); // overflow or underflow of exponential values...
		//std::cout<<"extremumList.size() = " << extremumList.size()<<std::endl;
		...
		if ( extremumList.size()<=1) {
			...
			dist_peaks.X = abs(extremumList[0].X - extremumList[1].X);	// distance in z between the two first best peaks in zPDF
			dist_peaks.Y = ( exp( extremumList[0].Y + lbins) - exp( extremumList[1].Y + lbins));// distance in pz
		...
		if ( nanVector ) {
			...
			Int32 N2 =  (   redshiftRange.GetEnd() - redshiftRange.GetBegin() )/redshiftStep   +1 ;
			Bool condi = (N2== nb_pz) ;
			if ( condi ){
				loc_z = 0;
			} else {
			 ...
			 * 			m_j =  sum [ p(z_i) * (   ( z_i - mu ) / sigma  )^j   ]
			 *
			 * 		SKEWNESS s  	= sum [ p(z_i) * (  ( z_i - mu ) / sigma  )^3   ]
			 * 		                    	= m_3 / sigma^3
			 * 		                    	= [ E[z^3] - 3*mu*sigma2 - mu^3 ] / sigma.^3
			 *
			 * 		KURTOSIS k   = sum [ p(z_i) * (  ( z_i - mu ) / sigma  )^4   ]
			 * 		                     = m_4 / sigma2^2
			 *
			 * ********************************************************************** */
			...
			for ( Int32 i=0; i<nb_pz; i++) {
				...
				zmoment3 += zpdf1d[i] * zcenter * zcenter * zcenter ; // = E [ (z - E[z] ) ^3 ]
				zmoment4 += zpdf1d[i] * zcenter * zcenter * zcenter * zcenter;  // = E [ (z - E[z] ) ^4 ]
			}
            ...
            if ( 0 )
			{
				...
				<< " \t [KM] dist_pz = " << dist_peaks.Y <<"\n"
				<< " \t [KM] 1st Peak (z="  <<  extremumList[0].X << " ; pz= " << exp( extremumList[0].Y + lbins) << ") \n"
				<< " \t [KM] 2nd Peak (z=" <<  extremumList[1].X << " ; pz= " << exp( extremumList[1].Y + lbins) << ") "
				<<std::endl;
			}
		}
	}
	...
	std::string desc_7 = "#z_in_R1";					// Region around zMAP [R1], credibility region (CR) 95% ;
	std::string desc_8 = "dz_in_R1";
	std::string desc_9 = "sum_p(z_in_R1|D,I)";
	std::string desc_10 = "sum_p(z_in_R2|D,I)";	//Region around zMAP [R2], crit dz=0.001;
	...
	auto zfeature_vect = std::shared_ptr<CPdfzFeatureResult>(new CPdfzFeatureResult());
...
Void CQualz::DisplayPrediction()
{
	...
	<<" \t* [zQual] associated posterior class prediction = " <<std::setprecision(10)<< m_predProba
	<< std::endl;

	/*
	std::cout<<std::setprecision(10)
	<<"\t* [Class C1] : \t PostProba = "<<m_posterior->data[0] <<" ;  Score = " <<m_score->data[0] <<"\n"
	<<"\t* [Class C2] : \t PostProba = "<<m_posterior->data[1] <<" ;  Score = " <<m_score->data[1] <<"\n"
	<<"\t* [Class C3] : \t PostProba = "<<m_posterior->data[2] <<" ;  Score = "  <<m_score->data[2] <<"\n"
	<<"\t* [Class C4] : \t PostProba = "<<m_posterior->data[3] <<" ;  Score = " <<m_score->data[3] <<"\n"
	<<"\t* [Class C5] : \t PostProba = "<<m_posterior->data[4] <<" ;  Score = " <<m_score->data[4] //<<"\n"
	<< std::endl;
	*/

	if ( disp_time ) {
		std::cout
		<< "\t* [zQual] extract features (in sec): " << to_simple_string(T0) <<"\n"
...
Int32 CQualz::GetNPeaksKM ( TPointList& peaks, Float64& zmap_estimate, Float64& lbins )
{
	...
	for ( i=0; i<centroids->size1; i++) {
		for ( j=0; j<centroids->size2; j++) {
			gsl_matrix_set ( centroids, i, j, minipz + ( ( ( Float64) rand() ) /RAND_MAX ) * (maxipz-minipz) );
		}
	}
	/* **********************************************************************
	...
	while ( iter<=maxIter &&  !cond_converge )
	{
		...
		for ( i=0; i<nClusters; i++) {
			...
			for ( h=0; h<nDesc; h++)	{
				...
				weighted_mean = gsl_stats_wmean ( weights, 1, temp3->data ,1, temp3->size );
		...
		cond_converge = ( index->size - counter ) == 0; // the algorithm is stuck on the best solution
	} // end_while
...
Float64 CQualz::GetDistanceKM ( gsl_vector *vect1, gsl_vector *vect2, std::string method )
{
	...
	if (method=="correlation") {
		...
		center_c = center_c/vect2->size; // vectors temp1 and temp2 have the same size

		Float64 xcorr_c = 0.0, normx_c = 0.0, normc_c = 0.0;
		for ( i=0; i<vect1->size; i++) 	{
			xcorr_c += ( vect1->data[i] - center_x ) * ( vect2->data[i] - center_c );
...
Void CQualz::ProjectPDF ( CClassifierStore& classifierStore )
{
	if ( nanVector ) {
		...
		GetPosteriorPred ( classifierStore );	 // store vector m_posterior, for all classes
		GetLabelPred ( classifierStore );       // decode score knowing the coding strategy (ECOC) to  derive the predicted label & its associated posterior proba
	}
}
...
Void CQualz::GetScorePred ( CClassifierStore& classifierStore )
{
	...
	for ( it=mapLearners.begin(); it!=mapLearners.end(); ++it)
	{
        ...
        if ( classifierStore.GetOptionClassifier()==1 ) {
			...
			gsl_vector_memcpy( prod_svAlpha, learner->m_SValpha);	// Copy alpha into temp

			prod_xsvt = GetProductKL ( m_Xc, sv_transpose );		// Vector c [1 x M] = PRODUCT( Vector a [1 x P] , Matrix b [size P x M]).	Element c[j] = SUM_i( a[i] * b[i,j] )
			gsl_vector_mul ( prod_svAlpha, learner->m_SVectorLabels ); // Vector c[1:K , j] = ( Vector a [1xK] .* Vector b [1xK] ). Element c[i,j] = a[i] * b[i]

			//gsl_blas_ddot( prod_xsvt, prod_svAlpha, &learner_sc);		// Float c =  ( Vector a [1xP] * Vector b [1xP] ). Element c = SUM_j(a[j] * b[j] )
			learner_sc = 0;
			for ( Int32 lig = 0; lig< prod_xsvt->size ; lig++) {
				learner_sc += ( gsl_vector_get(prod_xsvt, lig) ) *( gsl_vector_get(prod_svAlpha, lig) );
			}
...
Void CQualz::GetPosteriorPred ( CClassifierStore& classifierStore )
{
	...
	gsl_vector* p_iter1 = GetArgminKL ( classifierStore, r, w, p_init, dist_iter1 );
	gsl_vector* p_iter2 = GetArgminKL ( classifierStore, r, w, p_null, dist_iter2 );
	...
	for ( Int32 i = 0; i< m_posterior->size; i++) {
		...
		sump+=m_posterior->data[i]; //sump = gsl_blas_dasum( p ); // sum of absolute elements . the probability vector p is postive anyway
	}
...
gsl_vector*  CQualz::GetArgminKL ( CClassifierStore& classifierStore, gsl_vector* r, gsl_vector* w_learner, gsl_vector* p0, Float64& distance )
{
	...
	if ( std::isnan( p0->data[0] ) )
	{
		...
		gsl_matrix_add( M, classifierStore.GetCodingMatrixNeg()); //M = Mpos + Mneg .
		...
		{
			// UPDATE R_ESTIM
			tempPos = GetSumKL( GetTimesKL( classifierStore.GetCodingMatrixPos(), p ), opt_row );
			tempNeg = GetSumKL( GetTimesKL( classifierStore.GetCodingMatrixNeg(), p ) , opt_row );

			tempPosNeg = gsl_vector_alloc(tempPos->size);
			gsl_vector_memcpy( tempPosNeg, tempPos);
			gsl_vector_sub ( tempPosNeg, tempNeg );
			gsl_vector_div ( tempPos, tempPosNeg );    // r_estim = tempPos / (tempPos - tempNeg)
		...
		for ( Int32 i = 0; i< p->size; i++) {
			...
			sump+=p->data[i]; //sump = gsl_blas_dasum( p ); // sum of absolute elements . the probability vector p is postive anyway
		}
	...
	// FINAL ITERATION
	if ( 0) {
		std::cout<< " >>>>>>>>>> ITER FINAL  <<<<<<<<<<<< :  " << iter << std::endl;
		std::cout <<
				"\t >>   ["<<iter<<"] delta = "<<delta<<"\n"
...
gsl_vector* CQualz::GetNumDenKL( CClassifierStore& classifierStore, gsl_vector* r, gsl_vector* r_estim, gsl_vector* pold )
{
	...
	// COMPUTE:
	//					SUM_[ @times( Mpos, w.*r ) - @times( Mneg, w.*r ) ]      and      SUM_[ @times( Mpos, w.*rEstim ) - @times( Mneg, w.*rEstim ) ]
	...
	gsl_matrix* tempPos = GetTimesKL ( classifierStore.GetCodingMatrixPos(), wr ); // = @times( Mpos, w.*r )
	gsl_matrix* tempNeg = GetTimesKL ( classifierStore.GetCodingMatrixNeg(), wr1 ); // = @times( Mneg, w.*(1-r) )
	gsl_matrix* tempPos_ = GetTimesKL ( classifierStore.GetCodingMatrixPos(), wr_estim); // = @times( Mpos, w.*rEstim )
	gsl_matrix* tempNeg_= GetTimesKL ( classifierStore.GetCodingMatrixNeg(), wr_estim1 ); // = @times( Mneg, w.*(1-rEstim) )
	...
	for ( Int32 idrow = 0; idrow< K; idrow++) {// a sum in each row of the matrices
		s1 = 0.0;s2 = 0.0;
	...
	// UPDATE P
	if ( cond_counter>0 ) { // at least one number is !NAN & !0 in this vector
		...
		for ( Int32 idrow = 0; idrow< K; idrow++)	{
			...
			if ( conditio) {
				Float64 value = ((Float64) numer->data[idrow]) / ( (Float64) denom->data[idrow]);
...
Float64 CQualz::GetDistanceKL ( gsl_vector* r, gsl_vector* r_estim, gsl_vector* w_learner )
{
	...
	for ( Int32 i = 0; i< r->size; i++)
	{
		if ( r->data[i]> threshold ) {
			s1 += w_learner->data[i] * r->data[i] * log( r->data[i] / r_estim->data[i] );
		}
		if ( (1 - r->data[i] ) > threshold ) {
			s2 += w_learner->data[i] * (1-r->data[i]) * log( (1 - r->data[i]) / (1 - r_estim->data[i]) );
		}
	}
	distance = s1 + s2;
...
Float64 CQualz::GetNormKL ( gsl_matrix* m )
{
	...
	for ( Int32 j = 0; j < m->size2 ; j++) {
		...
		//s = gsl_blas_dasum ( temp );		// absolute sum \sum |x_i| of the elements of the vector x.
		s=0;
		for ( Int32 i = 0; i< temp->size; i++) {
			s+=temp->data[i]; //sump = gsl_blas_dasum( p ); // sum of absolute elements . the probability vector p is postive anyway
		}
		maxi = max( maxi, s );
	}
	Float64 normL1 = maxi;
...
Void CQualz::GetXc( gsl_vector* mu, gsl_vector* sigma )
{
	...
	std::string desc_3 = "std_p(z|D,I) ";							//2
	std::string desc_4 = "skewness_p(z|D,I)";			//3
	std::string desc_5 = "kurtosis_p(z|D,I)";				//4
	std::string desc_6 = "p(zmap_1d|D,I)";						//5
	std::string desc_7 = "#z_in_R1";									//6
	std::string desc_8 = "dz_in_R1";									//7
	std::string desc_9 = "sum_p(z_in_R1|D,I)";					//8
	...
	*/
	...
	for ( Int32 i=0; i<mu->size; i++) {
		if (selected[i]==-1) {
			xc = 2e-3; 	// unused descriptor for recent tests (but still in the old training set). No effect, bcse cste in the current config
		} else {
...
gsl_matrix* CQualz::GetTimesKL ( const gsl_matrix* m, gsl_vector* v )
{
	...
	for ( Int32 j = 0; j< m->size2 ; j++) {
		gsl_matrix_get_col ( temp, m, j );
		gsl_vector_mul ( temp, v ); // vector c[1:K , j] = ( Vector a [1xK] .* Vector b [1xK] ). Element c[i,j] = a[i] * b[i]
