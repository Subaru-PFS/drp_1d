...
void CTplcombinationSolveResult::Save( const CDataStore& store, std::ostream& stream ) const
{
    Float64 redshift;
    Float64 merit;
    Float64 evidence;
...
Bool CTplcombinationSolveResult::GetBestRedshiftPerTemplateString( const CDataStore& store, std::string& output ) const
{
    std::string scopeStr;
    if(m_type == nType_raw){
        scopeStr = "chisquare";
...
void CTplcombinationSolveResult::SaveLine( const CDataStore& store, std::ostream& stream ) const
{
    char tmpChar[256];
...
Bool CTplcombinationSolveResult::GetBestRedshift( const CDataStore& store, Float64& redshift, Float64& merit, std::string& tplName, Float64& amplitude, Float64& dustCoeff, Int32& meiksinIdx ) const
{
    std::string scopeStr;
    if(m_type == nType_raw){
        scopeStr = "chisquare";
 ...
 *
 **/
Bool CTplcombinationSolveResult::GetBestRedshiftFromPdf( const CDataStore& store, Float64& redshift, Float64& merit, Float64& evidence ) const
{
...
Int32 CTplcombinationSolveResult::GetEvidenceFromPdf(const CDataStore& store, Float64 &evidence) const
{
    //check if the pdf is stellar/galaxy or else
    std::string outputPdfRelDir  = "zPDF"; //default value set to galaxy zPDF folder
    std::string scope = store.GetScope( *this );
...
/**
 * @brief CTplcombinationSolveResult::GetBestModel
 ...
 * @return
 */
Int32 CTplcombinationSolveResult::GetBestModel(const CDataStore& store, Float64 z, std::string& tplName) const
{
    tplName = "-1";
...
Bool CTplcombinationSolveResult::GetRedshiftCandidates( const CDataStore& store,  std::vector<Float64>& redshiftcandidates, Int32 n_candidates) const
{
    Log.LogDebug( "CTplcombinationSolveResult:GetRedshiftCandidates" );
