#include <epic/redshift/operator/chisquareloglambda.h>
...
COperatorChiSquareLogLambda::COperatorChiSquareLogLambda( std::string calibrationPath )
{
    ...
    if(fileOpenFailed)
    {
        //Log.LogError("ChisquareLogLambda, unable to load the calzetti calib. file... aborting!");
...
Void COperatorChiSquareLogLambda::BasicFit(const CSpectrum& spectrum, const CTemplate& tpl, Float64* pfgTplBuffer,
                                ...
                                Float64& overlapRate, Float64& chiSquare, Float64& fittingAmplitude, Float64 &fittingDustCoeff, EStatus& status , std::string opt_interp, Float64 forcedAmplitude, Int32 opt_extinction, Int32 opt_dustFitting, CMask spcMaskAdditional)
{
    ...
    {
        Log.LogInfo("ChisquareLogLambda, spcMaskAdditional does not have the same size as the spectrum flux vector... (%d vs %d), aborting!", spcMaskAdditional.GetMasksCount(), spcFluxAxis.GetSamplesCount());
        status = nStatus_DataError;
        return ;
    }
    ...
    for(Int32 kDust=0; kDust<nDustCoeffs; kDust++)
    {
        ...
        Float64 coeffEBMV = m_dustCoeffStart + m_dustCoeffStep*(Float64)kDust;
        //Log.LogInfo("ChisquareLogLambda, fitting with dust coeff value: %f", coeffEBMV);
...
/**
 * @brief COperatorChiSquareLogLambda::prepareRebinedSpectrum
 * @param spectrum
 * @param redshift_step
 * @return
 */
Int32 COperatorChiSquareLogLambda::prepareRebinedSpectrum(const CSpectrum& spectrum, const TFloat64List& redshift_step)
{
    Int32 ret=0;
                          ...
                          Float64 overlapThreshold , std::vector<CMask> additional_spcMasks, std::string opt_interp, Int32 opt_extinction, Int32 opt_dustFitting)
{
    if(!m_dataCalzetti)
    {
        Log.LogError("ChisquareLogLambda, no calzetti calib. file loaded... aborting!");
        return NULL;
    }

//    if( spectrum.GetSpectralAxis().IsInLinearScale() == false || tpl.GetSpectralAxis().IsInLinearScale() == false )
//    {
//        Log.LogError("ChisquareLogLambda, input spectrum or template are not in log scale (ignored)");
//        //return NULL;
//    }
    ...
    if(opt_interp=="precomputedfinegrid"){
        ...
        {
            Log.LogError("ChisquareLogLambda, unable to allocate the precomputed fine grid buffer... aborting!");
            return NULL;
        }
    ...
    {
        Log.LogError("ChisquareLogLambda, using default mask, masks-list size (%d) didn't match the input redshift-list (%d) !)", additional_spcMasks.size(), sortedRedshifts.size());
    }
    ...
    {
        Log.LogInfo("ChisquareLogLambda, overlap warning for %s: minz=%.3f, maxz=%.3f", tpl.GetName().c_str(), overlapValidInfZ, overlapValidSupZ);
    }
...
/* @brief COperatorChiSquareLogLambda::getDustCoeff: get the dust coeff at a fixed resolution of 1A
* @param dustCoeff
* @param maxLambda
* @return
*/
