...
Float64 CDeltaz::Compute(TFloat64List merits, TFloat64List redshifts, Float64 redshift)
{
    Float64 winsize = 0.008;
    ...
    for( Int32 i2=0; i2<redshifts.size(); i2++ )
    {
        ...
        if(izmin == -1 && (redshift - winsize/2.0) <= redshifts[i2]){
            izmin = i2;
        }
        if(izmax == -1 && (redshift + winsize/2.0) <= redshifts[i2]){
            izmax = i2;
            break;
        }
    }
